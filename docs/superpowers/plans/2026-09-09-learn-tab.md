...
class LearnController < ApplicationController
  ...
  def language_buckets
    current_user.language == "mixed" ? %w[ruby_rails javascript] : [ current_user.language ]
  end
        ...
        <% concepts.each do |concept| %>
          <li class="learn-entry" data-concept="<%= concept %>">
            ...
            <% unless @references[[ concept, bucket[:key] ]]&.guide? %>
              <span class="muted learn-ungenerated"><%= t("learn.not_generated") %></span>
            <% end %>
          </li>
        <% end %>
      </ul>
...
<p class="muted"><%= t("learn.buckets.#{@bucket}") %></p>
...
<% else %>
  ...
  <div id="learn-guide" data-url="<%= prepare_learn_concept_path(bucket: @bucket, concept: @concept) %>">
    <p class="muted"><%= t("learn.no_guide") %></p>
...
```ruby
  ...
  describe "POST /learn/prepare" do
    ...
    it "enqueues one job per concept with no row at all" do
      expected = AiService::RAILS_CONCEPTS.size + AiService::ARCHITECTURE_CONCEPTS.size +
                 AiService::PLAN_REVIEW_CONCEPTS.size + AiService::AMBIGUITY_HUNT_CONCEPTS.size +
                 AiService::PSEUDOCODE_TO_CODE_CONCEPTS.size
...
Add `include ActiveJob::TestHelper` inside the `RSpec.describe "Learn"` block if `enqueued_jobs` is not already available, and confirm `ActiveJob::Base.queue_adapter` is `:test` in `spec/rails_helper.rb`; if it is not, add `before { ActiveJob::Base.queue_adapter = :test }` to this describe block rather than changing the global adapter.
...
Insert into `app/views/learn/index.html.erb`, directly after the `<p class="muted"><%= t("learn.intro") %></p>` line:
...
<div class="learn-filter">
  ...
  <p id="learn-empty" class="muted" hidden><%= t("learn.no_matches") %></p>
</div>
```
...
```css
    ...
    .learn-ungenerated { font-size: .75rem; }
