...
module Glossary
  TERMS = {
    ...
    "aggregate boundaries" => "The cluster of records that must stay consistent with each other, and the one object that is the only legal way to change any of them — a caller that updates a member directly can leave the set disagreeing with itself.",
    ...
    "cache key completeness" => "Including every input the cached value actually varies on in its key — a key that omits one silently hands unrelated requests each other's results until it expires.",
    ...
    "cognitive load" => "How much a reader has to hold in their head to change a piece of code safely — every extra thing they must know before touching it is part of the cost.",
    ...
    "dependency inversion" => "Depending on an abstraction both sides can agree on rather than on a concrete collaborator, so the high-level code isn't wired to one specific implementation.",
    "deterministic ordering" => "Guaranteeing a sort returns rows in the same order every time by adding a tiebreaker for duplicate sort keys — without one, paginated pages can repeat or skip rows.",
    ...
    "open closed" => "The idea that you should be able to add a new case by adding code, not by editing the code that already handles the existing cases.",
    "open/closed principle" => "The idea that you should be able to add a new case by adding code, not by editing the code that already handles the existing cases.",
    ...
    "semantic input validation" => "Checking that ingested data means what it claims, not just that it has the right shape — a value of the right type and a plausible magnitude can still be in the wrong unit, and ambiguous input should fail loudly rather than be guessed at.",
    ...
    "shallow module" => "A module whose interface costs about as much to learn as the implementation it hides, so wrapping it buys little; the opposite is a deep module with a simple interface over real complexity.",
    ...
    "temporal decomposition" => "Splitting a system by the order things happen rather than by the information each part owns, which scatters one piece of knowledge across the steps that touch it.",
    ...
    "unknown unknowns" => "The parts of a system a change needs to touch that the person making it has no way to know about — the worst kind of complexity, because no amount of care avoids what is invisible.",
    "ubiquitous language" => "One word per thing, meaning the same in conversation, in the code, and in the database — so a model named for a word the business does not use, or one word covering two different things, is a defect rather than a style choice.",
