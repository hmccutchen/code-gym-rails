...
- Worker database pool: the pool comes from `RAILS_MAX_THREADS` (default 5) and Solid Queue runs 3 worker threads on it. The judge fans out one thread per section, and each checks out a connection to write its `ApiUsage` row, so a generating user needs up to one connection per section on top of the worker threads. Set `RAILS_MAX_THREADS` on the worker service to at least 7 — 3 plus `ExerciseSection.slot_count`, which is 4 today — or a checkout timeout discards a verdict the provider has already billed
