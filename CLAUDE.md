...
- **Drills**: a user can mark one concept, or a whole Learn display group, as
  ...
  **The cap is `ConceptDrills::MAX_CONCURRENT`, two, counted in drills where a
  group is one.** The reasoning sits beside the constant. Not derived from any
  vocabulary size.
  ...
  **Drilling a paused concept ends the pause**, through the same exit an
  ...
  says so before the click — the one deliberate look at tier state on the
  Learn tab, since a silent override would be worse than naming it. A concept
  that reaches the paused tier while drilled keeps its drill but waits out
  the cooldown; `concepts_needing_reinforcement` skips paused rows whether
  drilled or not, and the page says it is waiting. Accepted parity with the
  history-derived entries: for a mixed-language user a drill in the other
  language can be offered on a day that cannot host it.
