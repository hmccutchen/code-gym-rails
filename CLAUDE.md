...
- **The Progress page shows the rung each concept is held at**, from the
  stamps above: `/progress`, a nav entry between Learn and History, grouped
  exactly as Learn groups the same concepts (`ConceptBucket.slice_for`,
  `ConceptGroup.grouped`), nothing regrouped. `RungLedger` is the one rule:
  for a concept, bucket and rung, the most recent answered, reviewed,
  un-eased section pitched at that rung; held when both ratings were
  favourable, the co-favourable rule `record_review!` applies; the standing
  ...
  navigation. No locked-kind marker: `eased` records whether easing was
  applied, which is the fact a lock changes, so a rung held in a locked kind
  and an unlocked one rest on the same standard.
